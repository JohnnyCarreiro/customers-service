...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ContactAddress')
    BEGIN
      CREATE TABLE ContactAddress (
        ...
        CONSTRAINT FK_ContactAddress_ContactId FOREIGN KEY (ContactId) REFERENCES Contact(Id),
        CONSTRAINT FK_ContactAddress_AddressId FOREIGN KEY (AddressId) REFERENCES Address(Id),
